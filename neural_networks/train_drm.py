...
from neural_networks.system_registry import (
    SystemType,
    get_system_config,
    get_transformation,
    get_visualization_bounds,
)
from neural_networks.drm_dataset import (
    create_data_loaders,
    TechSubstitutionDataset,
    SaddleSystemDataset,
    SocialTippingDataset,
    get_saddle_configuration,
)
...
def train_drm_model(config_path, multi_run=False):
    ...
    if not multi_run:
        print(
            f"Detected dimensions: obs_dim={obs_dim}, control_dim={control_dim}, value_dim={value_dim}"
        )
    ...
    if use_warmup and not multi_run:
        print(
            f"Using warmup for first {warmup_epochs} epochs (starting LR: {lr * 0.1:.2e})"
        )
    ...
    if collect_initial and not multi_run:
        ...
        metrics, dominant_states, grid_points, state_probs = (
        ...
        collect_softmax_rank_metrics(
            model, val_loader, device, epoch=0, history=history, verbose=verbose
        )
    ...
    for epoch in range(epochs):
        ...
        if not multi_run:
            ...
            print(
                f"  Batch Entropy: {train_batch_entropy:.4f}, Individual Entropy: {train_individual_entropy:.4f}"
            )
            print(
        ...
        if ((epoch + 1) % 5 == 0 or epoch == 0) and system_type == "saddle_system":
            ...
            if not multi_run:
                print(
                    f"Validation Probing - Discrete Accuracy: {intermediate_results['discrete_accuracy']:.4f}"
                )
                print(
                    f"Validation Probing - Unweighted Accuracy: {intermediate_results['discrete_accuracy_unweighted']:.4f}"
                )
        ...
        if not multi_run and (
            epoch == 2 or epoch == 5 or epoch == 10 or epoch == 20 or epoch == 50
        ):
            state_vis_path = os.path.join(
                output_dir, f"states_after{epoch}_{run_id}.png"
            )
            visualize_state_space(
                model=model,
                output_path=state_vis_path,
                ...
                points=points_config["points"] if points_config else None,
                angles_degrees=(
                    points_config["angles_degrees"] if points_config else None
                ),
                bounds=bounds,
            )
        ...
        if (epoch + 1) % collect_every_n_epochs == 0:
            collect_softmax_rank_metrics(
                model, val_loader, device, epoch=epoch + 1, history=history
            )
        ...
        if not multi_run and ((epoch + 1) % checkpoint_every == 0 or (epoch + 1) == 10):
            checkpoint_path = os.path.join(
                output_dir, f"drm_checkpoint_epoch{epoch+1}_{run_id}.pt"
            )
            torch.save(
                {
                    "epoch": epoch,
    ...
    if not multi_run and history["state_metrics"]:
        ...
        analysis_path = os.path.join(
            output_dir, f"state_evolution_analysis_{run_id}.png"
        )
    ...
    if not multi_run:
        ...
        print(
            f"  Loss: {test_loss:.4f} (State: {test_state_loss:.4f}, Value: {test_value_loss:.4f})"
        )
        print(f"  Entropy Loss: {test_entropy_loss:.4f}")
        print(
            f"  Batch Entropy: {test_batch_entropy:.4f}, Individual Entropy: {test_individual_entropy:.4f}"
        )
        if probing_results:
            print(
                f"  Linear Probing - Discrete Accuracy: {probing_results['discrete_accuracy']:.4f}"
            )
            print(
                f"  Linear Probing - Unweighted Accuracy: {probing_results['discrete_accuracy_unweighted']:.4f}"
            )
    ...
    if "softmax_rank_metrics" in history and history["softmax_rank_metrics"]:
        ...
        if not multi_run:
            print(
                "Post-processing: Computing global normalization for singular values..."
            )
        ...
        if not multi_run:
            ...
            rank_plot_path = os.path.join(
                output_dir, f"softmax_rank_evolution_{run_id}.png"
            )
            ...
            print(
                f"Final σ₂/σ₁ ratio: {final_metrics.get('logit_sv_ratio_2nd_to_1st', 0):.4f}"
            )
    ...
    if multi_run:
        ...
        return None, {
            ...
            "test_accuracy": test_metrics.get("prob_discrete_accuracy"),
        }
