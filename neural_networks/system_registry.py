...
SYSTEM_CONFIGS = {
    "tech_substitution": {
        ...
        "value_dim": {"market_share": 1, "90% market share": 1, "identity": 2},
        "default_value_loss": {
            "market_share": "mse",
            "90% market share": "binary_cross_entropy",
            "identity": "mse",
        },
        "transformation": {"type": "tangent", "params": {"x0": 3.0, "alpha": 0.5}},
        "value_sorting_functions": {
            "market_share": lambda values: values[:, 0],  # Single value, sort directly
            "identity": lambda values: values[:, 1]
            / values[:, 0],  # similar to angle / market share sorting
        },
    ...
    "saddle_system": {
        ...
        "value_dim": {"angle": 2, "identity": 2},
        "default_value_loss": {"angle": "angular", "identity": "mse"},
        "transformation": {"type": "logistic", "params": {"k": 0.5, "x_0": 0.0}},
        "value_sorting_functions": {
            "angle": lambda values: (
                torch.atan2(values[:, 0], values[:, 1]) * 180 / torch.pi
            )
            % 360,
            "identity": lambda values: values[:, 0]
            + values[:, 1],  # Sum of transformed components
        },
        "visualization_bounds": {
            "x_space": [(-5, 5), (-5, 5)],
    ...
    "social_tipping": {
        ...
        "default_value_loss": {"abs_distance": "mse", "identity": "mse"},
        "transformation": {
            "type": "identity",  # no transformation needed
            "params": {},
        },
        "value_sorting_functions": {
            "abs_distance": lambda values: values[
                :, 0
            ],  # Single distance value, sort directly
            "identity": lambda values: values[:, 0]
            + values[:, 1],  # Sum of x0 + x1 for sorting
        },
        "visualization_bounds": {
...
def get_value_sorting_function(system_type: SystemType, value_method: str):
    ...
    if value_method not in config["value_sorting_functions"]:
        raise ValueError(
            f"No sorting function for value method '{value_method}' in {system_type.value}"
        )
