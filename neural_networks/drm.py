...
class DiscreteRepresentationsModel(nn.Module):
    ...
    def update_target_encoder(self):
        ...
        with torch.no_grad():
            for src_param, tgt_param in zip(
                ...
                tgt_param.data.mul_(self.ema_decay).add_(
                    src_param.data, alpha=1 - self.ema_decay
                )
    ...
    def get_embeddings_and_logits(self, x, use_target=False):
        ...
        encoder_to_use = (
            self.target_encoder
            if self.use_target_encoder and use_target
            else self.encoder
        )
    ...
    def get_state_probs(
        self, x, training=True, hard=False, use_target=False, soft=False
    ):
        # Choose encoder
        encoder_to_use = (
            self.target_encoder
    ...
    def update_temperature(
        ...
        if epoch <= delay_epochs:
            ...
            self.current_temp = (
                self.initial_temp * (self.min_temp / self.initial_temp) ** progress
            )
    ...
    def forward(self, x, c, y, v_true, training=True):
        ...
        s_x = self._logits_to_probs(
            logits_x, training=training, use_target=False, hard=False, soft=False
        )

        # Get embeddings and state probs for next observation
        logits_y = self.get_logits(y, use_target=True)
        s_y = self._logits_to_probs(
            logits_y, training=training, use_target=True, hard=False, soft=False
        )
    ...
    def compute_values_for_all_states(self):
        ...
        """
        one_hot_states = torch.eye(
            self.num_states, device=next(self.parameters()).device
        )
    ...
    def sort_states_by_value(
        ...
        if sorted_indices is None:
            if system_type is None or value_method is None:
                raise ValueError(
                    "Must provide either sorted_indices or both system_type and value_method"
                )
            ...
            with torch.no_grad():
                values = self.compute_value(
                    one_hot_states
                )  # Shape: (num_states, value_dim)

            # Get sorting function and compute sort keys
            sorting_func = get_value_sorting_function(
                SystemType[system_type.upper()], value_method
            )
            ...
            for i, orig_idx in enumerate(sorted_indices):
                print(
                    f"  New state {i} <- Original state {orig_idx.item()} (value: {sort_keys[orig_idx].item():.2f})"
                )
    ...
    def _reorder_state_parameters(self, sorted_indices):
        ...
        with torch.no_grad():
            ...
            encoder_output_layer.weight.data = encoder_output_layer.weight.data[
                sorted_indices
            ]
            if encoder_output_layer.bias is not None:
                encoder_output_layer.bias.data = encoder_output_layer.bias.data[
                    sorted_indices
                ]
            ...
            if hasattr(self, "target_encoder") and self.target_encoder is not None:
                ...
                target_output_layer.weight.data = target_output_layer.weight.data[
                    sorted_indices
                ]
                if target_output_layer.bias is not None:
                    target_output_layer.bias.data = target_output_layer.bias.data[
                        sorted_indices
                    ]
            ...
            if hasattr(self.predictor, "__class__"):
                ...
                if predictor_class_name == "StandardPredictor":
                    ...
                    print(
                        f"Warning: Reordering not implemented for {predictor_class_name}"
                    )

            # 4. Reorder value network input layer
            value_input_layer = self.value_net[
                0
            ]  # First layer takes state probabilities
            # Reorder columns (input dimensions) corresponding to state probabilities
            value_input_layer.weight.data = value_input_layer.weight.data[
                :, sorted_indices
            ]
    ...
    def _reorder_standard_predictor(self, sorted_indices):
        ...
        predictor_input_layer = self.predictor.predictor[
            0
        ]  # First layer of sequential predictor
        ...
        predictor_input_layer.weight.data = torch.cat(
            [reordered_state_weight, control_weight], dim=1
        )

        # Output layer: produces next state probabilities - reorder rows
        predictor_output_layer = self.predictor.predictor[-1]  # Last layer
        predictor_output_layer.weight.data = predictor_output_layer.weight.data[
            sorted_indices
        ]
        if predictor_output_layer.bias is not None:
            predictor_output_layer.bias.data = predictor_output_layer.bias.data[
                sorted_indices
            ]
...
class BasePredictor(nn.Module):
    ...
    def __init__(
        self, num_states, control_dim, hidden_dim, control_format="continuous"
    ):
        super().__init__()
        self.num_states = num_states
