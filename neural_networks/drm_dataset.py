...
class BaseDataset(Dataset):
    ...
    def __init__(self, db_path, cache_data=True, verbose=False):
        ...
        if not data_tables:
            raise ValueError(
                "No data tables found in the database (only found configs?)"
            )
    ...
    def _validate_samples(self, num_samples=5, verbose=False):
        ...
        try:
            for i in range(min(num_samples, len(self))):
                ...
                if verbose:
                    print(
                        f"Sample {i}: {[d.numpy() if torch.is_tensor(d) else d for d in data]}"
                    )
...
class TechSubstitutionDataset(BaseDataset):
    ...
    def __init__(
        self, db_path, value_method="market_share", cache_data=True, verbose=False
    ):
    ...
    def f_v(self, y):
        ...
        if y.ndim == 1:
            ...
            raise ValueError(
                "Input must be a tuple of two values or an array of shape (n, 2)"
            )
        ...
        if self.value_method == "market_share":
            ...
            raise NotImplementedError(
                f"Method '{self.value_method}' is not implemented."
            )
...
class SaddleSystemDataset(BaseDataset):
    ...
    def __init__(
        ...
        if self.saddle_config:
            ...
            if self.num_saddles != expected_saddles:
                if verbose:
                    print(
                        f"WARNING: Inferred {self.num_saddles} saddles from data, "
    ...
    def _infer_num_saddles(self, verbose=False):
        ...
        if self.cached_data:
            ...
            if verbose:
                print(
                    f"Inferred {self.num_saddles} saddle points from cached control values: {sorted(unique_controls)}"
                )
        else:
            ...
            with Session(self.engine) as session:
                ...
                self.num_saddles = len(unique_controls)
                print(
                    f"Inferred {self.num_saddles} saddle points from control values: {unique_controls}"
                )
    ...
    def __getitem__(self, idx):
        ...
        if int(c0) >= self.num_saddles or int(c0) < 0:
            raise ValueError(
                f"Invalid control value {c0}. Must be in range [0, {self.num_saddles})"
            )
    ...
    def f_v(self, y):
        ...
        if self.value_method == "angle":
            ...
            raise NotImplementedError(
                f"Method '{self.value_method}' is not implemented for saddle system."
            )
    ...
    def _halfspace_for_batch(self, x_batch):
        ...
        for saddle_idx, (saddle_point, angle_deg) in enumerate(
            zip(saddle_points, angles_degrees)
        ):
...
class SocialTippingDataset(BaseDataset):
    ...
    def __init__(
        self, db_path, value_method="abs_distance", cache_data=True, verbose=False
    ):
        self.value_method = value_method
        super().__init__(db_path, cache_data=cache_data)
        self._validate_samples()
    ...
    def f_v(self, state):
        ...
        if self.value_method == "abs_distance":
            ...
            raise NotImplementedError(
                f"Method '{self.value_method}' is not implemented for social tipping system."
            )
...
def create_data_loaders(
    ...
    if value_method is None:
        ...
        raise ValueError(
            f"Invalid value method '{value_method}' for {system_type}. "
            f"Available: {system_config['value_methods']}"
        )

    # Create the appropriate dataset
    if system_type == "tech_substitution":
        dataset = TechSubstitutionDataset(
            db_path, value_method=value_method, verbose=verbose
        )
    elif system_type == "saddle_system":
        dataset = SaddleSystemDataset(
            db_path, value_method=value_method, verbose=verbose
        )
    elif system_type == "social_tipping":
        dataset = SocialTippingDataset(
            db_path, value_method=value_method, verbose=verbose
        )
    else:
    ...
    if verbose:
        ...
        print(
            f"Validation set: {len(val_indices)} samples (used for intermediate probing)"
        )
...
def get_saddle_configuration(db_path, verbose=True):
    ...
    try:
        with engine.connect() as conn:
            ...
            return {"saddle_points": saddle_points, "angles_degrees": angles_degrees}
