...
class StableDRMLoss(nn.Module):
    def __init__(
        ...
        if state_loss_type not in ["kl_div", "cross_entropy", "mse", "js_div"]:
            raise ValueError(
                f"Unsupported state loss type: {state_loss_type}. "
        ...
        if value_loss_type not in ["mse", "angular", "binary_cross_entropy"]:
            raise ValueError(
                f"Unsupported value loss type: {value_loss_type}. "
    ...
    def forward(
        ...
        if (
            self.value_loss_type in expected_dims
            and expected_dims[self.value_loss_type] is not None
        ):
        ...
        if s_x is not None:
            ...
            if self.use_entropy_reg:
                ...
                entropy_loss = (
                    batch_weight * (1.0 - batch_entropy)
                    + individual_weight * individual_entropy
                )
    ...
    def batch_entropy(self, state_probs):
        ...
        max_entropy = torch.log(
            torch.tensor(float(num_states), device=state_probs.device)
        )
    ...
    def _calculate_expected_value_loss(
        self,
        s_y_pred,
        v_pred_for_all_states,
        v_true,
        ...
        expected_v_pred = torch.matmul(
            s_y_pred, v_pred_for_all_states
        )  # <-- REVERT TO THIS
