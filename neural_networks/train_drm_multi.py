...
def _aggregate_epoch_dict_data(all_epoch_dicts):
    ...
    for key in metric_keys:
        ...
        for epoch_list in all_epoch_dicts:
            run_values = [
                epoch_dict.get(key) for epoch_dict in epoch_list if key in epoch_dict
            ]
        ...
        if all_values_for_key and all(
            len(vals) == len(all_values_for_key[0]) for vals in all_values_for_key
        ):
...
def aggregate_results(config_output_dir):
    ...
    if "test_metrics" in histories[0]:
        ...
        for key in test_keys:
            values = [
                hist["test_metrics"][key]
                for hist in histories
                if "test_metrics" in hist
                and key in hist["test_metrics"]
                and hist["test_metrics"][key] is not None
            ]
            if values:
    ...
    print(
        f"  aggregated['test_metrics'] keys: {list(aggregated['test_metrics'].keys())}"
    )
    if "prob_discrete_accuracy" in aggregated["test_metrics"]:
        print(
            f"  prob_discrete_accuracy found: {aggregated['test_metrics']['prob_discrete_accuracy']}"
        )
    else:
    ...
    if "prob_discrete_accuracy" in aggregated["test_metrics"]:
        mean_test_prob_discrete_accuracy = aggregated["test_metrics"][
            "prob_discrete_accuracy"
        ]["mean"]
        print(
            f"Primary metric (test_prob_discrete_accuracy): {mean_test_prob_discrete_accuracy:.4f}"
        )
...
def monitor_process(process, log_file, run_name, run_index, total_runs):
    ...
    try:
        ...
        if success:
            print(
                f"✓ RUN {run_index}/{total_runs}: {run_name} completed in {runtime:.1f}s"
            )
        else:
            print(
                f"✗ RUN {run_index}/{total_runs}: {run_name} failed in {runtime:.1f}s (exit code: {process.returncode})"
            )
        ...
        runtime = time.time() - start_time
        print(
            f"✗ RUN {run_index}/{total_runs}: {run_name} failed with exception in {runtime:.1f}s: {e}"
        )
...
def multi_train_drm_subprocess(
    config_path, output_dir, config_id, seeds, db_paths, max_parallel=None
):
    """
    ...
    while len(active_processes) < max_parallel and pending_configs:
        run_index, (config_file_path, run_name, override_values) = pending_configs.pop(
            0
        )
    ...
    while active_processes or pending_configs:
        ...
        if monitor_count % 10 == 0:
            print(
                f"[Monitor] Active: {len(active_processes)}, Pending: {len(pending_configs)}, Completed this check: {len(completed_processes)}"
            )

            # If no progress after many iterations, check process status
            if monitor_count > 50 and len(completed_processes) == 0:
                print(
                    "[DEBUG] No processes completing - checking first 3 process statuses:"
                )
                for i, (
                    process,
                    (log_file, run_name, run_index, total_runs),
                ) in enumerate(list(active_processes.items())[:3]):
                    try:
                        poll_result = process.poll()
                        print(
                            f"  Process {i+1} ({run_name}): poll()={poll_result}, pid={process.pid}"
                        )
                        ...
                        if psutil.pid_exists(process.pid):
                            ...
                            print(
                                f"    Status: {proc_info.status()}, CPU%: {proc_info.cpu_percent()}"
                            )
                        else:
        ...
        for process in completed_processes:
            ...
            if pending_configs:
                run_index, (config_file_path, run_name, override_values) = (
                    pending_configs.pop(0)
                )

                new_process, new_log_file = run_subprocess_training(
                    config_file_path, run_name, run_index, len(run_configs), log_dir
                )
                active_processes[new_process] = (
                    new_log_file,
                    run_name,
                    run_index,
                    len(run_configs),
...
def main():
    parser = argparse.ArgumentParser(
        description="Multi-run DRM training with subprocess execution"
    )
    parser.add_argument("config_path", help="Path to base YAML configuration file")
    parser.add_argument(
        "--output-dir",
    ...
    parser.add_argument(
        "--db-paths", required=True, help="Comma-separated list of database paths"
    )
    parser.add_argument(
        "--max-parallel",
        type=int,
    ...
    successful_runs, failed_runs, output_dir, primary_metric = (
        multi_train_drm_subprocess(
