...
def run_layer_probing(model, probing_loader, device, system_type, db_path, verbose=False):
...
def create_optimizer_with_bias_exclusion(model, optimizer_type, lr, weight_decay, verbose=False):
    """
...
def create_scheduler(optimizer, scheduler_type, epochs, warmup_epochs, 
                     use_warmup, min_lr, restart_period, restart_mult, verbose=False):
    """
...
def extract_hidden_and_logit_data(model, dataloader, device, max_samples=1000, verbose=False):
    """
...
def collect_softmax_rank_metrics(model, val_dataloader, device, epoch, history, max_samples=1000, verbose=False):
    """
